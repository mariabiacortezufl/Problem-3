#These two lines below provide a display name and $DISPLAY environment variable.
#Those were errors I had been getting without adding them at the very beggining of the scrit.
#Another option is to comment these two lines out and use ssh -Y when logging to the server.
...
#I used a 'try and except' because the first line was just the title of the columns in the dataset
#and I wanted to avoid that.
for line in fhand:
#Setting the string portions of the data set to be analyzed.
...
#Problem 2
#Necessary to import Pandas before using it.
import pandas as pd

#Opening the file with Pandas. Notice how different it is than how it is done in problem 1.
...
df.columns
#Print the row in the data set that cointains the highest water level.
...
i = 0
#Initiating the 'for loop' that reads row per row and finds the largest value across all the rows in the data set.
#I used a 'try and except' because the first row was just the title of the columns in the dataset
#and I wanted to avoid that.
for i, row in df.iterrows():
    try:
#Setting the date and time(0) column and the water level column(1) to be analyzed in the loop.
#For the water level column(1) we compute the difference between two consecutive measurements.
...
#Setting the ticks to be recovered within a bigger interval, diminishng the number of ticks shown in the graph.
